...
def v1_5_sub_src_path(payload: SubmissionConversionPayload) -> str:
...
class FileManager:
    ...
    def download_source(self, payload: ConversionPayload) -> Tuple[str, LocalFileObj]:
        ...
        if isinstance(payload, DocumentConversionPayload):
            ...
            # This path used to rely on `sub_src_path` via the `single_file` flag.
            # However, since Submission v1.5, virtually all submissions have >1 file (README.json)
            # Thus, simplifying assumption - we have a `.tar.gz` blob, unless we can't find it
            src = UngzippedFileObj(self.sub_src_store.to_obj(v1_5_sub_src_path(payload)))
    ...
    def clean_up_publish(self, payload: PublishPayload) -> None:
        try:
            ...
            pass
        try:
            shutil.rmtree(f"{self.local_publish_store.prefix}{payload.paper_id.idv}")
        except Exception:
            pass
