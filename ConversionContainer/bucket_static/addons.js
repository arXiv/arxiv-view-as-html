let create_header = () => {
    ...
    if (id === 'submission') {
        ...
        <div role="banner" class="header-message"> \
            <strong>Experimental HTML</strong>. Report rendering errors with the "Open Issue" button or click <strong>alt/option+Y</strong> to toggle accessible section reporting links. <a href="#footer">Reference all keyboard commands</a> in the footer. \
        </div> \
        <div></div>';
    } else {
        header.innerHTML =
...
let create_footer = () => {
    ...
    <div class="keyboard-glossary ltx_page_content"> \
        ...
        <ul> \
            ...
            <li>If using a screen reader, <strong>alt/option+Y</strong> will toggle individual reporting buttons at each section on and off. Useful when you want to report an issue just within a specific section, as highligting is not screen reader compatible.</li> \
